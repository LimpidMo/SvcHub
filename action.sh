...
api_save_config() {
    ...
    local has_si=0 has_sd=0 has_ts=0 has_sv=0 has_bc=0
    local si= sd= ts= sv= bc=
    while IFS= read -r line; do
        [ -z "$line" ] && continue
        key=${line%%=*}
        b64=${line#*=}
    ...
    write_config_json "$si" "$sd" "$ts" "$sv" "$bc" || { echo '{"success":false,"error":"配置写入失败"}'; exit 1; }
